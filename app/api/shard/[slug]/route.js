...
try {
  ...
  if(mode) existingShard.mode = mode;
   ...
   const activity =  await Activity.findOne({
    ...
    return NextResponse.json({message :"Shard updation error"}, {status: 500});
}
});
...
export const DELETE = auth(async (req, {params}) => {
    ...
    try {
        ...
        return NextResponse.json({message :"Shard deleted successfully"}, {status: 200});
        
    } catch (error) {
        console.log("Shard deletion error: ", error.message);
        return NextResponse.json({message :"Shard deletion error"}, {status: 500});
    }
...
export const PATCH = auth(async (req, {params}) => {
    ...
    try {
        ...
        return NextResponse.json({message :"Shard patched successfully"}, {status: 200});
