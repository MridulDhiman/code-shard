...
export const updateLikes = async (id, likes, likeStatus, email) => {
    try {
      ...
      if(likeStatus === "liked" && !existingShard.likedBy.includes(user._id.toString())) {
        existingShard.likes++;
      ...
      else if(likeStatus === "unliked" && existingShard.likedBy.includes(user._id.toString())) {
        existingShard.liked--;
...
export const addCommentToShard = async (msg, shardId, user, parent) => {
  try {
    ...
    if(parent === null) {
      commentDoc.threadId = commentDoc._id;
      await Shard.findOneAndUpdate({
        _id: shardId
      }, {
        $set: {
          commentThread: commentDoc._id
        }
      })
    }
      ...
      await Shard.findOneAndUpdate({
        ...
        $set: {
          commentThread: comment.threadId
        }
      })
    }
...
export const getFeedOfUser = async (user) => {
  ...
  try {
    ...
    const activites = await getRecentActivityofFollowing(existingUser.following);
